...
same fixture setup (isolated in-memory app, sample user, sample film) and the same
assertion style used across the codebase.
"""
...
def test_add_to_watchlist_nonexistent_film_raises(app, sample_user):
    ...
    This is the watchlist equivalent of
...
def test_add_to_watchlist_duplicate_raises(app, sample_user, sample_film):
    ...
    not silently create a duplicate entry (Comment 2).
    """
    with app.app_context():
...
def test_get_watchlist_returns_newest_first(app, sample_user):
    ...
    with app.app_context():
        ...
        # order (which would put "Alien" first).
